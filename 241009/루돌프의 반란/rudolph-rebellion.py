...
for t in range(1, m + 1):
    ...
    if closestIdx:
        ...
        elif closestX < rudolf[0]: moveX = -1
        if   closestY > rudolf[1]: moveY = 1
        elif closestY < rudolf[1]: moveY = -1
    ...
    if rudolf[0] == closestX and rudolf[1] == closestY:
        ...
        # 연쇄적으로 충돌이 일어난 가장 마지막 위치에서 시작해, 순차적으로 보드판에 있는 산타를 한칸씩 이동시킵니다.
        while not (lastX == firstX and lastY == firstY):
            beforeX = lastX - moveX
            beforeY = lastY - moveY
            if not is_inrange(beforeX, beforeY): break

            idx = board[beforeX][beforeY]
            if not is_inrange(lastX, lastY): is_live[idx] = False
            else:
    ...
    for i in range(1, p+1):
        # 장외 or 기절된 산타는 pass
        if not is_live[i] or stun[i] >= t: continue

        # 상우하좌 순으로 움직여서 움직일 방향 결정
        minDist = (pos[i][0] - rudolf[0]) ** 2 + (pos[i][1] - rudolf[1]) ** 2
        moveDir = -1
        for dir in range(4):
            nx = pos[i][0] + dx[dir]
            ny = pos[i][1] + dy[dir]
            # 산타가 움직이는 뱡항이 보드 밖 or 다른 산타가 있는 경우 skip
            if not is_inrange(nx, ny) or board[nx][ny] > 0:  continue
